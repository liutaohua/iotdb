...
public class OrNode implements Node {
  ...
  public Comparable nextObject() throws IOException {
    if (hasLeftValue() && !hasRightValue()) {
      ...
      return popAndFillObjectNextCache(
          leftValue.compareTo(rightValue) > 0,
          rightValue.compareTo(leftValue) > 0,
          leftValue,
          rightValue);
    }
    throw new IOException("no more data");
  ...
  private Comparable popAndFillObjectNextCache(
      boolean popLeft, boolean popRight, Comparable left, Comparable right) {
    if (popLeft) {
